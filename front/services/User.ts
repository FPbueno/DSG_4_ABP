...
class User {
    ...
    async create(name: string, mail: string,fone:string,pais:string, password: string): Promise<TokenProps | ErrorProps> {
        try {
            const { data } = await api.post("/cadastro", { name, mail,fone,pais,password });
            return data;
        } catch (error: any) {
            return error;
        }
